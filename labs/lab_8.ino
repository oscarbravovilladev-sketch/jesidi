...
// --- MAX4466 ---
#define MIC_PIN 9
const int MIC_SAMPLE_WINDOW = 50; // ms de muestreo para pico-a-pico
const int MIC_AVG_SAMPLES = 20;   // numero de lecturas para promediar
float dbHistory[20];
int dbIndex = 0;
bool dbFilled = false;
...
const long MQTT_MIN_INTERVAL = 200; // ms minimo entre publishes (max ~5/seg)
...
const long intervaloMic = 100; // ms entre lecturas de microfono
...
float readDBRaw() {
  ...
  int sigMax = 0;
  int sigMin = 4095;
  while (millis() - start < MIC_SAMPLE_WINDOW) {
...
float readDB() {
  dbHistory[dbIndex] = readDBRaw();
  dbIndex = (dbIndex + 1) % MIC_AVG_SAMPLES;
  if (!dbFilled && dbIndex == 0) dbFilled = true;
  int count = dbFilled ? MIC_AVG_SAMPLES : dbIndex;
  if (count == 0) return 0;
  float sum = 0;
  for (int i = 0; i < count; i++) sum += dbHistory[i];
  return sum / count;
}
...
const long BTN_LONG_PRESS = 2000;
...
void loop() {
  ...
  updateDht();
  if (millis() - lastMicRead >= intervaloMic) {
    lastMicRead = millis();
    cachedDB = readDB();
  }
  ...
  snprintf(mqttBuffer, sizeof(mqttBuffer),
    "{\"id\":%d,\"x\":%d,\"y\":%d,\"ang\":%.1f,\"hum\":%.1f,\"temp\":%.1f,\"dist\":%.0f,\"dB\":%.1f}",
    0, target.x, target.y, ang, cachedHum, cachedTemp, dist, cachedDB);
